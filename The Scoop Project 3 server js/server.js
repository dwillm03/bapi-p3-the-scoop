//import yaml-js as yaml from './nodule_modules/yaml-js';
...
function upvoteComment(url, request) {
  ...
  const voteUsername = request.body && request.body.username;
  const response = {};

  let comment = database.comments[commentId];
  if (!commentId || !comment || !voteUsername || !database.users[voteUsername])
  {
    response.status = 400;
    return response;
  }
  if (comment.downvotedBy.includes(voteUsername)) {
    comment.downvotedBy.splice(comment.downvotedBy.indexOf(voteUsername), 1);
    response.status = 200;
    response.body = {comment: comment}; //database.comments[commentId];
  }
  if (!comment.upvotedBy.includes(voteUsername)) {
    comment.upvotedBy.push(voteUsername);
    response.status = 200;
    response.body = {comment: comment}; //database.comments[commentId];
...
function downvoteComment(url, request)
{
  ...
  if (comment.upvotedBy.includes(voteUsername)) {
    comment.upvotedBy.splice(comment.upvotedBy.indexOf(voteUsername), 1);
    response.status = 200;
    response.body = {comment: comment}; //database.comments[commentId];
  }
  if (!comment.downvotedBy.includes(voteUsername)) {
    comment.downvotedBy.push(voteUsername);
    response.status = 200;
    response.body = {comment: comment}; //database.comments[commentId];
 }
  return response;
...
function updateComment(url, request) {
  ...
  {
    response.status = 400;
    //savedComment.body = savedComment.body;
  }
  ...
  else
  {
    ...
    response.body = {comment: savedComment.body};
    response.status = 200;
  }
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    //savedComment.commentIds.forEach(commentId =>
      ...
      const userCommentId = database.users[comment.username].commentIds;
      userCommentId.splice(userCommentId.indexOf(id), 1);

      const commentArticleId = database.articles[savedComment.articleId].commentIds;
